...
class QueryResolver:
    ...
    def __init__(self, preloader: Preloader, database_handler: DatabaseHandler, d=None) -> None:
    ...
    async def aroutes_for_as_set(self, set_name: str, ip_version: Optional[int]=None, exclude_sets: Set[str]=None) -> Set[str]:
        """
        Find all originating prefixes for all members of an AS-set. May be restricted
        to IPv4 or IPv6. Returns a set of all prefixes.
        """
        ...
        self._current_excluded_sets = exclude_sets if exclude_sets else set()
        self._current_set_maximum_depth = 0
        members = await self._arecursive_set_resolve({set_name})
        return self.preloader.routes_for_origins(members, self.sources, ip_version=ip_version)

    async def amembers_for_set(self, parameter: str, exclude_sets: Set[str]=None, depth=0, recursive=False) -> List[str]:
        """
        Find all members of an as-set or route-set, possibly recursively.
        Returns a list of all members, including leaf members.
        """
        ...
        self._current_set_maximum_depth = depth
        if not recursive:
            members, leaf_members = await self._afind_set_members({parameter})
            members.update(leaf_members)
        else:
            members = await self._arecursive_set_resolve({parameter})
        if parameter in members:
            members.remove(parameter)
    ...
    async def _arecursive_set_resolve(self, members: Set[str], sets_seen=None) -> Set[str]:
        """
        Resolve all members of a number of sets, recursively.
        ...
        resolved_as_members = set()
        sub_members, leaf_members = await self._afind_set_members(members)
        ...
        further_resolving_required = sub_members - set_members - sets_seen - resolved_as_members - self._current_excluded_sets
        new_members = await self._arecursive_set_resolve(further_resolving_required, sets_seen)
        set_members.update(new_members)

        return set_members

    async def _afind_set_members(self, set_names: Set[str]) -> Tuple[Set[str], Set[str]]:
        """
        Find all members of a number of route-sets or as-sets. Includes both
        direct members listed in members attribute, but also
        members included by mbrs-by-ref/member-of.
        ...
        for result in query_result:
            ...
            query = query.finalise_statement()
            referring_objects = await self.d.fetch_all(query=query)
            referring_objects = [dict(row) for row in referring_objects]
