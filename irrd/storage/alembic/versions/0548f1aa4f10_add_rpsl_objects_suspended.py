...
def upgrade():
    op.create_table(
        ...
        sa.Column('pk', postgresql.UUID(as_uuid=True), server_default=sa.text('gen_random_uuid()'), nullable=False),
    ...
    op.create_index(op.f('ix_rpsl_objects_suspended_mntners'), 'rpsl_objects_suspended', ['mntners'], unique=False)
    op.create_index(op.f('ix_rpsl_objects_suspended_object_class'), 'rpsl_objects_suspended', ['object_class'], unique=False)
