...
class RPSLRouteSetMemberField(RPSLTextField):
    ...
    def __init__(self, ip_version: int | None, *args, **kwargs) -> None:
...
class RPSLGenericNameField(RPSLTextField):
    ...
    def __init__(
        self, allowed_prefixes: list[str] | None = None, non_strict_allow_any=False, *args, **kwargs
    ) -> None:
...
class RPSLAuthField(RPSLTextField):
    ...
    def parse(
        ...
        messages.error(
            f"Invalid auth attribute: {value}: supported options are {hashers}, PGPKEY-xxxxxxxx and"
            f" {RPSL_MNTNER_AUTH_INTERNAL} for migrated objects"
        )
        return None
