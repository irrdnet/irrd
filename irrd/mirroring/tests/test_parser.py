...
class TestMirrorFullImportParser:
    def test_parse(self, monkeypatch, caplog):
        ...
        assert 'Full import for TEST: 9 objects read, 1 objects inserted, ignored 2 due to errors' in caplog.text
        assert 'ignored 1 due to object_class_filter' in caplog.text
        assert 'Ignored 5 objects found in full import for TEST due to unknown object classes' in caplog.text
...
class TestNRTMStreamParser:
    ...
    def test_test_parse_nrtm_v3_invalid_unexpected_source(self):
        ...
        assert 'Invalid NRTM source in START line: expected BADSOURCE but found TEST ' in str(ve)
    ...
    def test_test_parse_nrtm_v1_invalid_too_many_items(self):
        ...
        assert flatten_mock_calls(mock_dh) == [['force_record_serial_seen', ('TEST', 11012700), {}]]
