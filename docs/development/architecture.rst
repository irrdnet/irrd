...
NRTMv3 is done through special ``-g`` whois queries.
...
NRTMv3 queries through IRRd are whois queries, so their entry point is the
``WhoisQueryParser``. After validation of parameters and access rights,
the NRTMv3 data is generated by ``irrd.mirroring.NRTMv3_generator.NRTMv3Generator``.
This class queries the journal for the specific source, does validation
of the requested serials, and generates an NRTMv3 compliant output.
...
updates should be retrieved from NRTMv3, or a full new file import.
...
If updates should be retrieved over NRTMv3, the runner will call
``NRTMImportUpdateStreamRunner``, which retrieves the NRTMv3 update data
from the NRTMv3 source. The NRTM data is then parsed and validated by
